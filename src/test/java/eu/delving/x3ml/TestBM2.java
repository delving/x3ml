...
public class TestBM2 {
    ...
    public void testBM20() throws IOException {
        X3MLEngine engine = engine("/bm/test1.x3ml");
        X3MLEngine.Output output = engine.execute(document("/bm/20BMObjects-noEmptyTags.xml"), 
						  policy("/bm/00-generator-policy.xml"));
	
        Model expected = ModelFactory.createMemModelMaker().createModel("gumby");
        expected.read(TestBM2.class.getResourceAsStream("/bm/test1_rdfINIT.xml"), null, "RDF/XML-ABBREV");
